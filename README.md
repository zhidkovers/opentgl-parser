...
Скрипт собирает CSV-файлы со страниц 1–3 каталога открытых данных, парсит их через pandas (автоопределение разделителя и кодировки), сохраняет локально в JSON (массив объектов, `data/*.json`) и загружает в Google Таблицу. Каждому CSV-файлу соответствует отдельный лист, имя листа совпадает с именем файла (без расширения `.csv`).
...
После каждого запуска в папке `data/` создаются JSON-файлы — по одному на каждый CSV. Формат — массив объектов:

```json
[
    {
        "№ п/п": "1",
        "Адрес установки таксофона": "г. Тольятти, б-р Здоровья, 25",
        "Абонентский номер": "88482660756"
    }
]
```
...
├── data/                 # JSON-файлы (результат парсинга)
